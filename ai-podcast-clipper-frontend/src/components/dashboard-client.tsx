...
export function DashboardClient({
    ...
    const finalizeUpload = async (uploadedFileId: string) => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            await finalizeUpload(uploadedFileId);
    ...
    const handleSubmit = async (url: string) => {
        ...
        try {
            const {success} = await downloadYouTubeVideo(url);

            if (!success) throw new Error("Failed to queue YouTube download");

            toast.success("YouTube video queued", {
                description:
                    "We're downloading and processing it now. Check the status below.",
                duration: 5000,
            });

            router.refresh(); // reflect the new "queued" row in uploadedFiles
        } catch (error) {
            toast.error("Upload failed", {
                description:
                    "There was a problem downloading the YouTube video. Please try again.",
            });
        } finally {
            setUploading(false);
        }
    ...
    return (
        <div className="mx-auto flex max-w-5xl flex-col space-y-6 px-4 py-8">
            ...
            <YoutubeForm onSubmit={handleSubmit} isSubmitting={uploading}/>
                        ...
                        <CardContent>
                            ...
                            >
                                {(dropzone: DropzoneState) => (
                                    <>
                                        <div
                                            className="flex flex-col items-center justify-center space-y-4 rounded-lg p-10 text-center">
